...
extern "C" {
  ...
  NimBLEAdvertisementData WiFiScan::GetUniversalAdvertisementData(EBLEPayloadType Type) {
    ...
    switch (Type) {
      ...
      default: {
        esp32m_println("Please Provide a Company Type");
        break;
      }
    }
      ...
      void onResult(BLEAdvertisedDevice *advertisedDevice) {
        ...
        if (wifi_scan_obj.currentScanMode == BT_SCAN_ALL) {
          if (buf >= 0)
          {
            ...
            esp32m_print(" RSSI: ");
            esp32m_print(advertisedDevice->getRSSI());
            ...
            if(advertisedDevice->getName().length() != 0)
            {
              ...
              esp32m_print(advertisedDevice->getName().c_str());
              
            }
            else
            {
              display_string.concat(advertisedDevice->getAddress().toString().c_str());
              esp32m_print(advertisedDevice->getAddress().toString().c_str());
            }
  ...
  class bluetoothScanSkimmersCallback: public BLEAdvertisedDeviceCallbacks {
      void onResult(BLEAdvertisedDevice *advertisedDevice) {
        ...
        if (buf >= 0)
        {
          ...
          esp32m_print(" RSSI: ");
          esp32m_println(advertisedDevice->getRSSI());
        }
      }
  };
#endif
...
int WiFiScan::clearStations() {
  ...
  stations->clear();
  esp32m_println("stations: " + (String)stations->size());
...
int WiFiScan::clearAPs() {
  ...
  esp32m_println("access_points: " + (String)access_points->size());
  return num_cleared;
}
...
int WiFiScan::clearSSIDs() {
  ...
  ssids->clear();
  esp32m_println("ssids: " + (String)ssids->size());
  return num_cleared;
}
...
bool WiFiScan::addSSID(String essid) {
  ...
  ssids->add(s);
  esp32m_println(ssids->get(ssids->size() - 1).essid);
...
/*void WiFiScan::joinWiFi(String ssid, String password)
{
  ...
  if ((WiFi.status() == WL_CONNECTED) && (ssid == connected_network) && (ssid != "")) {
    ...
    esp32m_println("Already connected. Disconnecting...");
    WiFi.disconnect();
  }
  ...
  esp32m_print("Connecting to WiFi");
  ...
  esp32m_print("IP address: ");
  esp32m_println(WiFi.localIP());
  this->wifi_initialized = true;
}*/
...
void WiFiScan::initWiFi(uint8_t scan_mode) {
  ...
  if (scan_mode != WIFI_SCAN_OFF) {
    //esp32m_println(F("Initializing WiFi settings..."));
    this->changeChannel();
    ...
    //esp32m_println(F("Initialization complete"));
  }
}
...
void WiFiScan::StopScan(uint8_t scan_mode)
{
  ...
  (currentScanMode == BT_SCAN_SKIMMERS))
  {
    ...
    #endif
    //esp32m_print("display_buffer->size(): ");
    esp32m_println(display_obj.display_buffer->size());
...
void WiFiScan::RunLoadAPList() {
    ...
    if (!file) {
      esp32m_println("Could not open /APs_0.log");
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
    ...
    if (error) {
      esp32m_print("JSON deserialize error: ");
      esp32m_println(error.c_str());
      file.close();
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
        display_obj.tft.setFreeFont(NULL);
    ...
    #endif
    esp32m_print("Loaded APs:");
    esp32m_println((String)access_points->size());
  #endif
}
...
void WiFiScan::RunLoadSSIDList() {
    ...
    if (!log_file) {
      esp32m_println("Could not open /SSIDs_0.log");
      #ifdef HAS_SCREEN
        display_obj.tft.setTextWrap(false);
        display_obj.tft.setFreeFont(NULL);
        display_obj.tft.setCursor(0, 100);
    ...
    esp32m_print("Loaded SSIDs: ");
    esp32m_println((String)ssids->size());
  #endif
}
...
void WiFiScan::RunEvilPortal(uint8_t scan_mode, uint16_t color)
{
  ...
  //  esp32m_println("Could not successfully start EvilPortal. Setting WIFI_SCAN_OFF...");
  //  this->StartScan(WIFI_SCAN_OFF, TFT_MAGENTA);
  //  return;
  //}
  //else
  //  esp32m_println("Setup EvilPortal. Current mode: " + this->currentScanMode);
  this->wifi_initialized = true;
  initTime = millis();
}
...
void WiFiScan::RunAPScan(uint8_t scan_mode, uint16_t color)
{
  ...
  esp32m_println(text_table4[9] + (String)access_points->size());
  #ifdef HAS_SCREEN
    display_obj.TOP_FIXED_AREA_2 = 48;
    display_obj.tteBar = true;
    display_obj.print_delay_1 = 15;
  ...
  void WiFiScan::RunLvJoinWiFi(uint8_t scan_mode, uint16_t color) {
    ...
    display_obj.tft.setRotation(1);
      ...
      esp32m_println("Using TFT Shield");
    #else if defined(TFT_DIY)
      uint16_t calData[5] = { 213, 3469, 320, 3446, 1 }; // Landscape TFT DIY
      esp32m_println("Using TFT DIY");
    #endif
    #ifdef HAS_ILI9341
...
void WiFiScan::RunGPSInfo() {
    ...
    esp32m_println("Refreshing GPS Data on screen...");
    #ifdef HAS_SCREEN
    ...
    // Display to serial
    esp32m_println("==== GPS Data ====");
    if (gps_obj.getFixStatus())
      esp32m_println("  Good Fix: Yes");
    else
      esp32m_println("  Good Fix: No");
      
    if(text != "") esp32m_println("      Text: " + text);

    esp32m_println("Satellites: " + gps_obj.getNumSatsString());
    esp32m_println("  Accuracy: " + (String)gps_obj.getAccuracy());
    esp32m_println("  Latitude: " + gps_obj.getLat());
    esp32m_println(" Longitude: " + gps_obj.getLon());
    esp32m_println("  Altitude: " + (String)gps_obj.getAlt());
    esp32m_println("  Datetime: " + gps_obj.getDatetime());
  #endif
}
...
void WiFiScan::RunPacketMonitor(uint8_t scan_mode, uint16_t color)
{
  ...
  esp32m_println("Running packet scan...");
  esp_wifi_init(&cfg);
  esp_wifi_set_storage(WIFI_STORAGE_RAM);
  esp_wifi_set_mode(WIFI_MODE_NULL);
  esp_wifi_start();
...
void WiFiScan::RunEapolScan(uint8_t scan_mode, uint16_t color)
{
  ...
  if (err != 0)
  {
    esp32m_print("could not set protocol : err=0x");
    esp32m_println(err, HEX);
  }
  ...
  if (err != 0)
  {
    esp32m_print("AP config set error, Maurauder SSID might visible : err=0x");
    esp32m_println(err, HEX);
  }
    ...
    if (gps_obj.getGpsModuleStatus()) {
      ...
      if (n > 0) {
        for (int i = 0; i < n; i++) {
          ...
          esp32m_print((String)this->mac_history_cursor + " | " + wardrive_line);
...
void WiFiScan::RunBluetoothScan(uint8_t scan_mode, uint16_t color)
{
    ...
    esp32m_println("Started BLE Scan");
    this->ble_initialized = true;
    initTime = millis();
  #endif
}
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      if (src == "de:ad:be:ef:de:ad") {
        ...
        delay(random(0, 10));
        esp32m_print("RSSI: ");
        esp32m_print(snifferPacket->rx_ctrl.rssi);
        esp32m_print(" Ch: ");
        esp32m_print(snifferPacket->rx_ctrl.channel);
        esp32m_print(" BSSID: ");
        esp32m_print(addr);
        ...
        {
          esp32m_print((char)snifferPacket->payload[i + 38]);
          //display_string.concat((char)snifferPacket->payload[i + 38]);
          if (isAscii(snifferPacket->payload[i + 38]))
            essid.concat((char)snifferPacket->payload[i + 38]);
          else
            esp32m_println("Got non-ascii character: " + (String)(char)snifferPacket->payload[i + 38]);
        }
        //essid.concat("\": \"\"}}");
...
void WiFiScan::apSnifferCallbackFull(void* buf, wifi_promiscuous_pkt_type_t type) {  
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        mac_match = true;
        //esp32m_print("Checking ");
        //esp32m_print(addr);
        //esp32m_println(" against " + (String)access_points->get(i).essid);

        
        for (int x = 0; x < 6; x++) {
          //esp32m_println((String)snifferPacket->payload[x + 10] + " | " + (String)access_points->get(i).bssid[x]);
          if (snifferPacket->payload[x + 10] != access_points->get(i).bssid[x]) {
            mac_match = false;
            //esp32m_println("MACs do not match");
            break;
          }
        }
        if (mac_match) {
      ...
      if (!in_list) {
        ...
        esp32m_print(access_points->size());
        esp32m_print(" ");
        esp32m_print(esp_get_free_heap_size());
      ...
      if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
        ...
        for (int i = 0; i < access_points->size(); i++) {
          if (access_points->get(i).selected) {
            ...
            if (found) {
              //esp32m_println("Received beacon from " + access_points->get(i).essid + ". Checking RSSI...");
        ...
        if ((targ_ap.rssi + 5 < snifferPacket->rx_ctrl.rssi) || (snifferPacket->rx_ctrl.rssi + 5 < targ_ap.rssi)) {
          ...
          esp32m_println((String)access_points->get(targ_index).essid + " RSSI: " + (String)access_points->get(targ_index).rssi);
          return;
        }
      }
            ...
            if (do_save) {
              if (n > 0) {
                for (int i = 0; i < n; i++) {
                  ...
                  esp32m_print(" -> ");
                  esp32m_println(wifi_scan_obj.security_int_to_string(WiFi.encryptionType(i)).c_str());
                }
              }
...
void WiFiScan::stationSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
  ...
  esp32m_print((String)stations->size() + ": ");
  
  char sta_addr[] = "00:00:00:00:00:00";
  
  if (ap_is_src) {
    esp32m_print("ap: ");
    esp32m_print(ap_addr);
    esp32m_print(" -> sta: ");
    getMAC(sta_addr, snifferPacket->payload, 4);
    esp32m_println(sta_addr);
  }
  else {
    esp32m_print("sta: ");
    getMAC(sta_addr, snifferPacket->payload, 10);
    esp32m_print(sta_addr);
    esp32m_print(" -> ap: ");
    esp32m_println(ap_addr);
  }
...
void WiFiScan::rawSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
    ...
    if ((targ_ap.rssi + 5 < snifferPacket->rx_ctrl.rssi) || (snifferPacket->rx_ctrl.rssi + 5 < targ_ap.rssi)) {
      targ_ap.rssi = snifferPacket->rx_ctrl.rssi;
      access_points->set(targ_index, targ_ap);
      esp32m_print((String)access_points->get(targ_index).essid + " RSSI: " + (String)access_points->get(targ_index).rssi);
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x40) && (buf == 0))
    {
      if (wifi_scan_obj.currentScanMode == WIFI_SCAN_PROBE) {
        ...
        esp32m_print(addr);
        display_string.concat(addr);
        esp32m_print(" Requesting: ");
        display_string.concat(" -> ");
        for (int i = 0; i < snifferPacket->payload[25]; i++)
        {
          esp32m_print((char)snifferPacket->payload[26 + i]);
    ...
    if (wifi_scan_obj.currentScanMode == WIFI_SCAN_ACTIVE_LIST_EAPOL) {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        if (access_points->get(i).selected) {
          ...
          if (found) {
            esp32m_println("Received beacon from " + access_points->get(i).essid + ". Deauthenticating...");
            break;
          }
        }
      }
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along every point on the x axis and do something, start over when finished
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      for (uint8_t b = 0; b < BUTTON_ARRAY_LEN; b++)
      {
        ...
        {
          esp32m_println("Bro, key pressed");
          //do_break = true;
        }
        ...
        if (display_obj.key[b].justReleased())
        {
          ...
          if (b == 4) {
            if (set_channel > 1) {
              esp32m_println("Shit channel down");
              set_channel--;
              delay(70);
          ...
          // Channel + button pressed
            ...
            if (set_channel < MAX_CHANNEL) {
              esp32m_println("Shit channel up");
              set_channel++;
              delay(70);
              display_obj.tft.fillRect(127, 0, 193, 28, TFT_BLACK);
              display_obj.tftDrawChannelScaleButtons(set_channel);
            ...
            esp32m_println("Exiting packet monitor...");
      ...
      if (currentTime - initTime >= (GRAPH_REFRESH * 5)) {
        ...
        if (y_pos_x >= HEIGHT_1) {
          esp32m_println("Max EAPOL number reached. Adjusting...");
          num_eapol = 0;
        }
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along every point on the x axis and do something, start over when finished
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      if (currentTime - initTime >= GRAPH_REFRESH) {
        //esp32m_println("-----------------------------------------");
        //esp32m_println("Time elapsed: " + (String)(currentTime - initTime) + "ms");
        x_pos += x_scale;
        initTime = millis();
        ...
        //esp32m_println("num_beacon: " + (String)num_beacon);
        //esp32m_println("num_deauth: " + (String)num_deauth);
        //esp32m_println(" num_probe: " + (String)num_probe);
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  else if (currentScanMode == WIFI_SCAN_ACTIVE_EAPOL)
  {
    ...
    {
      initTime = millis();
      //esp32m_print("packets/sec: ");
      //esp32m_println(packets_sent);
      String displayString = "";
      String displayString2 = "";
      displayString.concat(text18);
      displayString.concat(packets_sent);
